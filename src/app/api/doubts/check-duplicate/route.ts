...
export async function POST(req: Request) {
  try {
    ...
    try {
      ...
      console.error("Embedding vector duplicate check path failed, falling back to LLM:", err);
    }

    // Fallback: LLM similarity detection if pgvector embeddings return no direct matches
...
Do not include any explanation or markdown.`;
    ...
    try {
      ...
      console.error("Groq API failed during duplicate check:", getSafeErrorDetails(err));
    ...
    try {
      ...
      console.error("Failed to parse Groq duplicate response:", raw);
      return NextResponse.json({ similarDoubts: [] });
    }
