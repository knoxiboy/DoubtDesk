    ...
    <header className="sticky inset-x-0 top-0 z-50 bg-white/80 dark:bg-black/80 backdrop-blur-xl border-b border-slate-200 dark:border-zinc-900 transition-colors duration-500">
      <div className="max-w-7xl mx-auto h-16 sm:h-20 flex items-center justify-between px-4 sm:px-6 lg:px-8 gap-4">
        ...
        <div className="hidden md:flex items-center gap-4">
          ...
          <SignedIn>
            <Link href="/dashboard" className="text-sm font-medium text-slate-700 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-white transition-colors duration-300">
              Dashboard
            </Link>
            {appUser?.role === 'admin' && (
              <Link href="/admin" className="text-sm font-medium text-red-600 dark:text-red-400 hover:underline transition-colors duration-300">
                Admin
              </Link>
            )}
            <Link href="/profile" className="text-sm font-medium text-slate-600 dark:text-zinc-300 hover:text-blue-600 dark:hover:text-white transition-colors duration-300">
              Profile
            </Link>
            {appUser?.role && (
            ...
            <UserButton
              ...
              appearance={{
                elements: {
                  userButtonAvatarBox: "w-9 h-9 border border-slate-200 dark:border-zinc-800 shadow-sm"
                }
              }}
            />
          </SignedIn>
          ...
          <SignedIn>
            <div className="flex items-center justify-between bg-slate-50 dark:bg-zinc-950 p-4 rounded-2xl border border-slate-100 dark:border-zinc-900">
              <div className="flex flex-col gap-0.5 text-left">
                <span className="text-xs font-semibold text-slate-400 dark:text-zinc-500 uppercase tracking-wider">Account Active</span>
                <span className="text-sm font-bold text-slate-800 dark:text-zinc-200">Session Controls</span>
              </div>
              <UserButton
                afterSignOutUrl="/"
                appearance={{
                  elements: {
                    userButtonAvatarBox: "w-10 h-10 border border-slate-200 dark:border-zinc-800 shadow-sm"
                  }
                }}
              />
            </div>
