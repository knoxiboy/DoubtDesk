...
export async function getDashboardAnalytics(db: any, params: GetDashboardAnalyticsParams) {
    const { email, classroomId } = params;

    // Check if user is blocked
    const { isBlocked } = await checkUserBlock(email);
    if (isBlocked) {
    ...
    if (classroomId) {
        ...
        if (!membership) {
            throw new ServiceError(403, "Access denied: not a member of this classroom");
        }
